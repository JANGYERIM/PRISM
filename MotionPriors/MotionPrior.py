...
def load_MotionPrior(model_ckpt_path, model_cfg):
    if model_cfg.model.name in ["Momask_RVQVAE", "Momask_VQVAE"]:
        ...
        return model
    elif model_cfg.model.name == "MoScale_HRVQVAE":
        opt = model_cfg.model
...
class MotionPriorWrapper(nn.Module):
    ...
    def get_z(self, motion, sample=False):
        ...
        elif net.__class__.__name__ == 'HRVQVAE':
            m_lens = torch.full((motion.shape[0],), motion.shape[1], device=motion.device)
            code_idx, all_codes, accumulated_fhat = net.encode(motion, m_lens=m_lens, perturb_rate=[0.0, 0.0], codebook=None, train=False)
            z = accumulated_fhat
