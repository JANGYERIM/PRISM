...
class Unet1DforFlowDecoder(Module):
    def __init__(
        self,
        dim=384,
        init_dim = None,
        ...
        self.init_proj = nn.Conv1d(channels + c_proj_dim, dim, 1)
        self.up_conv_c = up_conv_c
        ...
        # latent conditions
        # z -> (Bs, N/4, 512) -> (BS, N, 512) -> (Bs, N, c_proj_dim)
        # motion -> (Bs, N, 263)
        # z + motion => (Bs, N, 263 + c_proj_dim)
        if up_conv_c == "Transposed" or up_conv_c == True:
            self.condition_projection = nn.Sequential(
                nn.ConvTranspose1d(c_in_dim, c_in_dim, 2, 2), 
                nn.GELU(),
                nn.ConvTranspose1d(c_in_dim, c_proj_dim, 2, 2),
                nn.GELU(),
            )
        elif up_conv_c == "Upsample" or up_conv_c == False:
            self.condition_projection = nn.ModuleList()
            self.condition_projection.append(
                nn.Sequential(
                    nn.Conv1d(c_in_dim, c_in_dim, 5, stride=1, padding=2, padding_mode="replicate"),
                    nn.LeakyReLU(0.2, True),
                    nn.BatchNorm1d(c_in_dim),
                )
            )
            self.condition_projection.append(
                nn.Sequential(
                    nn.Conv1d(c_in_dim, c_proj_dim, 5, stride=1, padding=2, padding_mode="replicate"),
                    nn.LeakyReLU(0.2, True),
                    nn.BatchNorm1d(c_proj_dim),
                )
            )
        elif up_conv_c == "Nearest_and_linear":
            self.condition_projection = nn.Sequential(
                nn.Upsample(scale_factor = 4, mode = 'nearest'),# upsample in time
                # pointwise conv
                nn.Conv1d(c_in_dim, c_proj_dim, 1),
            )
        elif up_conv_c == "Repeat_and_linear":
            self.condition_projection = nn.Conv1d(c_in_dim, c_proj_dim, 1)
            
        elif up_conv_c == "Repeat_and_stack_and_linear":
            self.condition_projection = nn.Sequential(
                StackLinear(quant_factor=2, seq_first=False, unstack=False), # (B, dim, T) -> (B, dim, T//4, 4) -> (B, dim*4, T//4)
                nn.Conv1d(c_in_dim*4, c_in_dim*4, 1),
                StackLinear(quant_factor=2, seq_first=False, unstack=True), # (B, T//4, in_dim*4) -> (B, T, in_dim)
                nn.Conv1d(c_in_dim, c_proj_dim, 1),
            )
        else:
            raise NotImplementedError(f"up_conv_c {up_conv_c} not implemented")
        # elif up_conv_c == "repeate_and_linear":
        #     self.condition_projection = nn.Sequential(
        #         nn.re
    ...
    def forward(self, x, times, y,  padding_mask=None, text_embedding=None, t5_embedding=None, t5_padding_mask=None):
        ...
        if "Repeat" in self.up_conv_c:
            y = y.repeat_interleave(4, dim = -1)
            
        if self.up_conv_c == "Upsample":
            for i, module in enumerate(self.condition_projection):
                y = module(y)
                y = y.repeat_interleave(2, dim=-1)
            c = y
        else:
            c = self.condition_projection(y) # b, c_proj_dim, n
            
        x = torch.cat((x, c), dim = 1) # (b, channels+c_proj_dim, n)
